...
function SourceForm({ onClose, onDone, supportedDbTypes, source }: { onClose: () => void; onDone: () => void; supportedDbTypes: string[]; source?: Source }) {
  ...
  const testConnection = async () => { setTesting(true); setError(''); setTestResult(''); try { await api('/api/sources/test', { method: 'POST', body: JSON.stringify(form) }); setTestResult('연결 테스트에 성공했습니다. 아직 저장되지는 않았습니다.') } catch (e) { setError((e as Error).message) } finally { setTesting(false) } }
  const submit = async (e: FormEvent) => { e.preventDefault(); setBusy(true); setError(''); try { const payload = { ...form }; delete payload.id; delete payload.status; delete payload.last_tested_at; delete payload.created_at; delete payload.options; for (const key of ['password','service_account_json','ssh_password','ssh_private_key','ssh_private_key_passphrase','ssl_ca_cert','ssl_cert','ssl_key']) if (!payload[key]) delete payload[key]; await api(source ? `/api/sources/${source.id}` : '/api/sources', { method: source ? 'PUT' : 'POST', body: JSON.stringify(payload) }); onDone() } catch (e) { setError((e as Error).message) } finally { setBusy(false) } }
