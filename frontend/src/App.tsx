...
import {
  Activity, CalendarClock, Check, ChevronDown, CircleAlert, Database, Download, Eye, Gauge,
  KeyRound, LayoutDashboard, LockKeyhole, LogOut, Menu as MenuIcon, Play, Plus,
  RefreshCw, Search, Server, ShieldCheck, TableProperties, Trash2, Users, X, Zap,
} from 'lucide-react'
import { api, ApiError, downloadFile, fmt } from './api'
import type { Job, Menu, Permission, Role, Run, Source, User } from './types'

type Page = 'dashboard' | 'sources' | 'metadata' | 'jobs' | 'users' | 'roles' | 'account'
...
const dbNames: Record<string, string> = { postgresql: 'PostgreSQL', mysql: 'MySQL', mariadb: 'MariaDB', mssql: 'SQL Server', oracle: 'Oracle', sqlite: 'SQLite', db2: 'IBM Db2', bigquery: 'Google BigQuery' }
const dbPorts: Record<string, number> = { postgresql: 5432, mysql: 3306, mariadb: 3306, mssql: 1433, oracle: 1521, db2: 50000 }
...
function SourceForm({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  const [form, setForm] = useState<any>({ name: '', db_type: 'postgresql', host: 'localhost', port: 5432, database: '', username: '', password: '', options: { dataset: '', location: 'US' }, service_account_json: '', ssh_enabled: false, ssh_port: 22, ssh_auth_type: 'private_key' })
  const [error, setError] = useState(''); const [busy, setBusy] = useState(false)
  const set = (key: string, value: any) => setForm((f: any) => ({ ...f, [key]: value }))
  const setOption = (key: string, value: any) => setForm((f: any) => ({ ...f, options: { ...f.options, [key]: value } }))
  ...
  const isBigQuery = form.db_type === 'bigquery'
  return <Modal title="데이터베이스 연결 추가" subtitle="접속 정보는 암호화되어 안전하게 저장됩니다." onClose={onClose} wide><form onSubmit={submit}>
    <div className="form-section"><h4>기본 접속 정보</h4><div className="form-grid">
      <label>연결 이름<input required placeholder="예: 운영계 PostgreSQL" value={form.name} onChange={e => set('name', e.target.value)}/></label>
      <label>데이터베이스 종류<select value={form.db_type} onChange={e => { const type = e.target.value; setForm({ ...form, db_type: type, port: dbPorts[type], ssh_enabled: type === 'bigquery' ? false : form.ssh_enabled }) }}>{Object.entries(dbNames).map(([v,l]) => <option value={v} key={v}>{l}</option>)}</select></label>
      {form.db_type === 'sqlite' ? <label className="span-2">DB 파일 경로<input required placeholder="C:\data\sample.db" value={form.database} onChange={e => set('database', e.target.value)}/></label>
      : isBigQuery ? <>
        <label>Google Cloud 프로젝트 ID<input required placeholder="my-gcp-project" value={form.database} onChange={e => set('database', e.target.value)}/></label>
        <label>데이터셋 (선택)<input placeholder="analytics" value={form.options.dataset} onChange={e => setOption('dataset', e.target.value)}/></label>
        <label>리전<input required placeholder="US 또는 asia-northeast3" value={form.options.location} onChange={e => setOption('location', e.target.value)}/></label>
        <label className="span-2">서비스 계정 JSON<textarea required rows={8} placeholder={'{\n  "type": "service_account",\n  "project_id": "..."\n}'} value={form.service_account_json} onChange={e => set('service_account_json', e.target.value)}/><small>JSON 원문은 암호화해서 저장합니다. 메타데이터 조회 권한이 필요합니다.</small></label>
      </> : <>
        <label>호스트<input required value={form.host} onChange={e => set('host', e.target.value)}/></label><label>포트<input required type="number" value={form.port || ''} onChange={e => set('port', Number(e.target.value))}/></label>
        <label>데이터베이스 / 서비스명<input required value={form.database} onChange={e => set('database', e.target.value)}/></label><label>사용자명<input required value={form.username} onChange={e => set('username', e.target.value)}/></label>
        <label className="span-2">비밀번호<input type="password" value={form.password} onChange={e => set('password', e.target.value)}/></label>
      </>}
    </div></div>
    {!['sqlite', 'bigquery'].includes(form.db_type) && <div className="form-section"><div className="toggle-row"><div><h4>SSH 터널 사용</h4><p>점프 서버를 통해 사설망 DB에 안전하게 접속합니다.</p></div><button type="button" className={`toggle ${form.ssh_enabled ? 'on' : ''}`} onClick={() => set('ssh_enabled', !form.ssh_enabled)}><i/></button></div>{form.ssh_enabled && <div className="form-grid tunnel"><label>SSH 호스트<input required value={form.ssh_host || ''} onChange={e => set('ssh_host', e.target.value)}/></label><label>SSH 포트<input type="number" value={form.ssh_port} onChange={e => set('ssh_port', Number(e.target.value))}/></label><label>SSH 사용자<input required value={form.ssh_username || ''} onChange={e => set('ssh_username', e.target.value)}/></label><label>인증 방식<select value={form.ssh_auth_type} onChange={e => set('ssh_auth_type', e.target.value)}><option value="private_key">개인키</option><option value="password">비밀번호</option></select></label>{form.ssh_auth_type === 'private_key' ? <><label className="span-2">개인키 (PEM)<textarea required rows={5} placeholder="-----BEGIN OPENSSH PRIVATE KEY-----" value={form.ssh_private_key || ''} onChange={e => set('ssh_private_key', e.target.value)}/></label><label className="span-2">키 암호 (선택)<input type="password" value={form.ssh_private_key_passphrase || ''} onChange={e => set('ssh_private_key_passphrase', e.target.value)}/></label></> : <label className="span-2">SSH 비밀번호<input required type="password" value={form.ssh_password || ''} onChange={e => set('ssh_password', e.target.value)}/></label>}</div>}</div>}
    {error && <div className="alert"><CircleAlert size={16}/>{error}</div>}<div className="modal-actions"><button type="button" className="ghost" onClick={onClose}>취소</button><button className="primary" disabled={busy}>{busy && <RefreshCw className="spin" size={16}/>}연결 저장</button></div>
  </form></Modal>
}
...
function Sources({ toast }: { toast: (m: string) => void }) {
  ...
  return <><PageHead eyebrow="DATA MANAGEMENT" title="DB 접속 관리" description="메타데이터를 수집할 데이터베이스 연결과 인증 방식을 관리합니다." action={<button className="primary" onClick={() => setOpen(true)}><Plus size={17}/>새 연결</button>}/>
    <section className="panel"><div className="toolbar"><div className="search"><Search size={17}/><input placeholder="연결 이름, 호스트 검색" value={query} onChange={e => setQuery(e.target.value)}/></div><span className="count">총 {data.length}개 연결</span></div>{loading ? <Loading/> : filtered.length ? <div className="cards">{filtered.map(s => <article className="source-card" key={s.id}><div className="source-top"><div className={`db-logo db-${s.db_type}`}>{dbNames[s.db_type]?.slice(0,2) || 'DB'}</div><div><h3>{s.name}</h3><p>{dbNames[s.db_type]}</p></div><Status value={s.status}/></div><dl><div><dt>엔드포인트</dt><dd>{s.db_type === 'sqlite' ? s.database : s.db_type === 'bigquery' ? `${s.database} / ${(s.options.dataset as string) || '전체 데이터셋'}` : `${s.host}:${s.port}`}</dd></div><div><dt>데이터베이스</dt><dd>{s.database || '—'}</dd></div><div><dt>인증</dt><dd>{s.db_type === 'bigquery' ? <><KeyRound size={14}/> 서비스 계정</> : s.ssh_enabled ? <><KeyRound size={14}/> SSH {s.ssh_auth_type === 'private_key' ? '개인키' : '비밀번호'}</> : <><LockKeyhole size={14}/> DB 인증</>}</dd></div><div><dt>최근 테스트</dt><dd>{fmt(s.last_tested_at)}</dd></div></dl><div className="card-actions"><button onClick={() => test(s.id)}><Zap size={15}/>연결 테스트</button><button className="danger-icon" onClick={() => remove(s.id)} title="삭제"><Trash2 size={16}/></button></div></article>)}</div> : <Empty title="등록된 데이터베이스가 없습니다" text="새 연결을 추가하면 스키마 메타데이터 수집을 시작할 수 있습니다."/>}</section>{open && <SourceForm onClose={() => setOpen(false)} onDone={() => { setOpen(false); reload(); toast('데이터베이스 연결을 저장했습니다.') }}/>}</>
}

function JobForm({ sources, onClose, onDone }: { sources: Source[]; onClose: () => void; onDone: () => void }) {
  const [form, setForm] = useState<any>({ name: '', data_source_id: sources[0]?.id || '', schedule_type: 'cron', cron: '0 2 * * *', interval_minutes: 60, schemas: [], collect_storage: false, is_active: true }); const [error,setError]=useState('')
  const submit = async (e: FormEvent) => { e.preventDefault(); try { await api('/api/jobs',{method:'POST',body:JSON.stringify(form)}); onDone() } catch(e){setError((e as Error).message)} }
  return <Modal title="수집 스케줄 생성" subtitle="실행 주기와 수집할 메타데이터 범위를 지정하세요." onClose={onClose}><form onSubmit={submit}><div className="form-section"><div className="form-grid one"><label>작업 이름<input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="예: 운영계 매일 수집"/></label><label>데이터 소스<select required value={form.data_source_id} onChange={e=>setForm({...form,data_source_id:Number(e.target.value)})}><option value="">선택하세요</option>{sources.map(s=><option value={s.id} key={s.id}>{s.name}</option>)}</select></label><label>수집 옵션<select value={form.collect_storage?'storage':'basic'} onChange={e=>setForm({...form,collect_storage:e.target.value==='storage'})}><option value="basic">기본 메타데이터 수집</option><option value="storage">기본 + 스토리지 증가량 수집</option></select><small>스토리지 옵션은 테이블·인덱스 용량과 이전 수집 대비 증감량을 기록합니다.</small></label><label>실행 방식<select value={form.schedule_type} onChange={e=>setForm({...form,schedule_type:e.target.value})}><option value="cron">Cron 스케줄</option><option value="interval">주기 실행</option><option value="manual">수동 실행만</option></select></label>{form.schedule_type==='cron'&&<label>Cron 표현식<input required value={form.cron} onChange={e=>setForm({...form,cron:e.target.value})}/><small>분 시 일 월 요일 — 예: 매일 새벽 2시 = 0 2 * * *</small></label>}{form.schedule_type==='interval'&&<label>실행 간격 (분)<input type="number" min="1" value={form.interval_minutes} onChange={e=>setForm({...form,interval_minutes:Number(e.target.value)})}/></label>}<label>대상 스키마 (선택)<input placeholder="public, sales — 비우면 시스템 스키마 제외 전체" onChange={e=>setForm({...form,schemas:e.target.value.split(',').map(x=>x.trim()).filter(Boolean)})}/></label></div></div>{error&&<div className="alert">{error}</div>}<div className="modal-actions"><button type="button" className="ghost" onClick={onClose}>취소</button><button className="primary">스케줄 저장</button></div></form></Modal>
}

function Jobs({ toast }: { toast: (m:string)=>void }) {
  const jobs=useLoad<Job[]>('/api/jobs',[],[]), sources=useLoad<Source[]>('/api/sources',[],[]), runs=useLoad<Run[]>('/api/runs',[],[]); const [open,setOpen]=useState(false)
  ...
  return <><PageHead eyebrow="AUTOMATION" title="수집 스케줄" description="스키마 수집 작업의 실행 주기와 결과를 관리합니다." action={<button className="primary" disabled={!sources.data.length} onClick={()=>setOpen(true)}><Plus size={17}/>스케줄 생성</button>}/><section className="panel table-panel"><div className="responsive-table"><table><thead><tr><th>작업 이름</th><th>데이터 소스</th><th>수집 옵션</th><th>스케줄</th><th>다음 실행</th><th>상태</th><th></th></tr></thead><tbody>{jobs.data.map(j=><tr key={j.id}><td><b>{j.name}</b><small>스키마 {j.schemas.length?j.schemas.join(', '):'전체'}</small></td><td>{sourceName(j.data_source_id)}</td><td><span className="tag">{j.collect_storage?'스토리지 증가량':'기본'}</span></td><td><code>{j.schedule_type==='cron'?j.cron:j.schedule_type==='interval'?`${j.interval_minutes}분마다`:'수동'}</code></td><td>{fmt(j.next_run_at)}</td><td><span className={`status ${j.is_active?'status-success':'status-unchecked'}`}><i/>{j.is_active?'활성':'비활성'}</span></td><td className="row-actions"><button title="지금 실행" onClick={()=>run(j.id)}><Play size={15}/></button><button title="삭제" onClick={()=>remove(j.id)}><Trash2 size={15}/></button></td></tr>)}</tbody></table>{!jobs.data.length&&<Empty title="수집 스케줄이 없습니다" text={sources.data.length?'첫 수집 스케줄을 생성해 보세요.':'먼저 데이터베이스 연결을 등록해 주세요.'} icon={CalendarClock}/>}</div></section><section className="panel"><PanelHead title="실행 이력" text="최근 100건" action={<button className="icon-btn" onClick={runs.reload}><RefreshCw size={16}/></button>}/><div className="run-list compact">{runs.data.map(r=><div className="run-row" key={r.id}><div className="run-dot"><Activity size={15}/></div><div><b>{jobs.data.find(j=>j.id===r.job_id)?.name||`작업 #${r.job_id}`}</b><p>{fmt(r.started_at)} · {r.object_count}개 객체 {r.error_message&&`· ${r.error_message}`}</p></div><Status value={r.status}/></div>)}{!runs.data.length&&<p className="inline-empty">아직 실행 이력이 없습니다.</p>}</div></section>{open&&<JobForm sources={sources.data} onClose={()=>setOpen(false)} onDone={()=>{setOpen(false);jobs.reload();toast('수집 스케줄을 생성했습니다.')}}/>}</>
}

function Metadata({toast}:{toast:(m:string)=>void}) {
  const {data,loading}=useLoad<any[]>('/api/metadata',[],[]); const [selected,setSelected]=useState<number|null>(null); const [query,setQuery]=useState(''); const current=data.find(x=>x.id===(selected||data[0]?.id));
  const [exporting,setExporting]=useState(false); const exportExcel=async()=>{if(!current)return;setExporting(true);try{await downloadFile(`/api/metadata/${current.id}/export.xlsx`);toast('스키마 정보를 엑셀로 내려받았습니다.')}catch(e){toast((e as Error).message)}finally{setExporting(false)}}
  const tables=useMemo(()=>current?.payload.schemas.flatMap((s:any)=>s.tables.map((t:any)=>({...t,schema:s.name})))||[],[current]); const filtered=tables.filter((t:any)=>`${t.schema}.${t.name}`.toLowerCase().includes(query.toLowerCase()))
  return <><PageHead eyebrow="DATA MANAGEMENT" title="스키마 정보" description="수집된 데이터베이스 구조와 컬럼 정보를 탐색합니다."/><div className="metadata-layout"><aside className="panel source-tree"><h3>데이터 소스</h3>{data.map(x=><button className={current?.id===x.id?'active':''} onClick={()=>setSelected(x.id)} key={x.id}><Database size={17}/><span><b>{x.payload.source}</b><small>{x.payload.db_type} · {fmt(x.captured_at)}</small></span><ChevronDown size={15}/></button>)}{!data.length&&!loading&&<p>수집된 정보 없음</p>}</aside><section className="panel metadata-main">{current?<><div className="toolbar"><div><h3>{current.payload.source}</h3><p>{current.payload.schemas.length}개 스키마 · {tables.length}개 테이블{current.payload.storage_summary&&` · 총 ${fmtBytes(current.payload.storage_summary.total_bytes)} · 증감 ${fmtBytes(current.payload.storage_summary.growth_bytes)}`}</p></div><div className="metadata-actions"><div className="search"><Search size={17}/><input placeholder="스키마, 테이블 검색" value={query} onChange={e=>setQuery(e.target.value)}/></div><button className="primary export-button" disabled={exporting} onClick={exportExcel}>{exporting?<RefreshCw className="spin" size={15}/>:<Download size={15}/>}엑셀 다운로드</button></div></div><div className="schema-list">{filtered.map((table:any)=><details key={`${table.schema}.${table.name}`}><summary><div className="table-icon"><TableProperties size={16}/></div><div><b>{table.name}</b><small>{table.schema} · {table.columns.length} columns{table.storage&&` · ${fmtBytes(table.storage.total_bytes)}`}</small></div>{table.storage?.growth_bytes!=null&&<span className={`growth ${table.storage.growth_bytes>0?'up':table.storage.growth_bytes<0?'down':''}`}>{table.storage.growth_bytes>0?'+':''}{fmtBytes(table.storage.growth_bytes)}</span>}<ChevronDown size={16}/></summary>{table.storage&&<div className="storage-bar"><span><small>데이터</small><b>{fmtBytes(table.storage.data_bytes)}</b></span><span><small>인덱스</small><b>{fmtBytes(table.storage.index_bytes)}</b></span><span><small>전체</small><b>{fmtBytes(table.storage.total_bytes)}</b></span><span><small>이전 대비</small><b>{table.storage.growth_bytes==null?'기준값':`${table.storage.growth_bytes>0?'+':''}${fmtBytes(table.storage.growth_bytes)}`}</b></span><span><small>예상 행 수</small><b>{table.storage.row_estimate?.toLocaleString()??'—'}</b></span></div>}<div className="columns"><div className="column-head"><span>컬럼</span><span>타입</span><span>NULL</span><span>키</span></div>{table.columns.map((c:any)=><div className="column-row" key={c.name}><span><b>{c.name}</b>{c.comment&&<small>{c.comment}</small>}</span><code>{c.type}</code><span>{c.nullable?'YES':'NO'}</span><span>{table.primary_key?.constrained_columns?.includes(c.name)?<span className="key"><KeyRound size={12}/>PK</span>:'—'}</span></div>)}</div></details>)}</div></>:<Empty title="수집된 스키마 정보가 없습니다" text="수집 스케줄을 실행하면 데이터 자산을 여기서 탐색할 수 있습니다." icon={TableProperties}/>}</section></div></>
}

function AccountPage({user,toast}:{user:User;toast:(m:string)=>void}) {
  const [form,setForm]=useState({current_password:'',new_password:'',confirm_password:''}); const [error,setError]=useState(''); const [busy,setBusy]=useState(false)
  const submit=async(e:FormEvent)=>{e.preventDefault();setError('');if(form.new_password!==form.confirm_password){setError('새 비밀번호 확인이 일치하지 않습니다.');return}setBusy(true);try{await api('/api/auth/password',{method:'POST',body:JSON.stringify({current_password:form.current_password,new_password:form.new_password})});setForm({current_password:'',new_password:'',confirm_password:''});toast('접속 비밀번호를 변경했습니다.')}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
  return <><PageHead eyebrow="MY ACCOUNT" title="내 계정" description="계정 정보와 관리 콘솔 접속 비밀번호를 관리합니다."/><div className="account-layout"><section className="panel account-profile"><div className="account-avatar">{user.name.slice(0,1)}</div><h3>{user.name}</h3><p>{user.email}</p><div>{user.roles.map(role=><span className="tag" key={role}>{role}</span>)}</div></section><section className="panel account-password"><PanelHead title="접속 비밀번호 변경" text="현재 비밀번호 확인 후 새 비밀번호로 변경합니다."/><form onSubmit={submit}><div className="form-section"><div className="form-grid one"><label>현재 비밀번호<input required type="password" autoComplete="current-password" value={form.current_password} onChange={e=>setForm({...form,current_password:e.target.value})}/></label><label>새 비밀번호<input required type="password" minLength={8} maxLength={128} autoComplete="new-password" value={form.new_password} onChange={e=>setForm({...form,new_password:e.target.value})}/><small>8자 이상으로 입력하세요.</small></label><label>새 비밀번호 확인<input required type="password" minLength={8} maxLength={128} autoComplete="new-password" value={form.confirm_password} onChange={e=>setForm({...form,confirm_password:e.target.value})}/></label></div></div>{error&&<div className="alert"><CircleAlert size={16}/>{error}</div>}<div className="modal-actions"><button className="primary" disabled={busy}>{busy?<RefreshCw className="spin" size={16}/>:<ShieldCheck size={16}/>}비밀번호 변경</button></div></form></section></div></>
}
...
function UsersPage({toast}:{toast:(m:string)=>void}){const users=useLoad<User[]>('/api/admin/users',[],[]),roles=useLoad<Role[]>('/api/admin/roles',[],[]);const[open,setOpen]=useState(false);return <><PageHead eyebrow="ADMINISTRATION" title="사용자 관리" description="관리 콘솔에 접근할 사용자와 활성 상태를 관리합니다." action={<button className="primary" onClick={()=>setOpen(true)}><Plus size={17}/>사용자 추가</button>}/><section className="panel table-panel"><div className="responsive-table"><table><thead><tr><th>사용자</th><th>역할</th><th>상태</th><th>등록일</th></tr></thead><tbody>{users.data.map(u=><tr key={u.id}><td><div className="user-cell"><span>{u.name.slice(0,1)}</span><div><b>{u.name}</b><small>{u.email}</small></div></div></td><td>{u.roles.map(r=><span className="tag" key={r}>{r}</span>)}</td><td><span className={`status ${u.is_active?'status-success':'status-failed'}`}><i/>{u.is_active?'활성':'중지'}</span></td><td>—</td></tr>)}</tbody></table></div></section>{open&&<UserForm roles={roles.data} onClose={()=>setOpen(false)} onDone={()=>{setOpen(false);users.reload();toast('사용자를 추가했습니다.')}}/>}</>}
...
export default function App() {
  const [user,setUser]=useState<User|null>(null); const [checking,setChecking]=useState(true); const [page,setPage]=useState<Page>(()=>(location.hash.slice(1) as Page)||'dashboard'); const [mobile,setMobile]=useState(false); const [toast,setToast]=useState('')
