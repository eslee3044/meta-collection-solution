...
function SourceForm({ onClose, onDone, supportedDbTypes, source }: { onClose: () => void; onDone: () => void; supportedDbTypes: string[]; source?: Source }) {
  ...
  const cleanPayload = () => { const payload = { ...form }; delete payload.id; delete payload.status; delete payload.last_tested_at; delete payload.created_at; delete payload.options; for (const key of ['password','service_account_json','ssh_password','ssh_private_key','ssh_private_key_passphrase','ssl_ca_cert','ssl_cert','ssl_key']) if (!payload[key]) delete payload[key]; return payload }
